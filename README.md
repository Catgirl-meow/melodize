...
- **Synced lyrics** — time-synced LRC from LRClib, auto-scrolls with the song
- **Queue management** — drag to reorder, play next, add to queue
- **Home screen** — time-aware greeting, snap-to-card carousels, M3 Expressive entrance animations
...
A small Python service that runs on your Navidrome host, unlocking server-management features:
...
With the ARL configured, long-pressing a recommendation or tapping the download icon in search will save the full lossless FLAC to your Navidrome server via the companion.
